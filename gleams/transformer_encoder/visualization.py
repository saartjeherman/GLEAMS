...
def plot_distance_distributions(
    ...
    Use after every epoch on train and val sets to track separation. Returns
...
def _format_hyperparams_subtitle(df: pd.DataFrame) -> str:
    ...
    train_epoch row gives the full architecture; the val_epoch row contributes
    only its pair count (val data size).
    """
    train_rows = df[df['split'] == 'train_epoch']
    val_rows = df[df['split'] == 'val_epoch']
    if len(train_rows) == 0:
        return ""
    ...
    if len(val_rows) > 0:
        optim_.append(f"val_pairs={int(val_rows.iloc[0]['n_pairs']):,}")
    return " · ".join(arch) + "\n" + " · ".join(optim_)
...
def plot_loss_curves(
    ...
    """Render train + val loss vs. epoch from a `loss_log.csv` file.

    Reads rows with `split == 'train_epoch'` and `split == 'val_epoch'`. Annotates
    ...
    if len(val) > 0:
        ax.plot(val['epoch'], val['loss'], marker='o', linewidth=1.8,
                color=_NEGATIVE_COLOR, label='Validation')
